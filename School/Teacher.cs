...
public class Teacher : Human
{
    ...
    public Teacher(string fullname = "Без имени", int age = 0, double height = 0, SubjectType subject = SubjectType.Spells)
    {
        if (age < 0 || height < 0 || (int)subject > Enum.GetNames(typeof(SubjectType)).Length || (int)subject < 1)
        {
    ...
    public string GetAllTeacherMarksInfo()
    {
        ...
        if(result == "")
        {
            result = "Учитель не выставлял оценок\n";
        }
